	...
	init: function(options){
		...
		$(_config.targetCls).each(function(index,item){
			...
			$(item).keyup(function(e){	
				...
				// 如果输入框值为空的话 那么下拉框隐藏
				if(targetVal == ''||(targetVal.substring(targetVal.length-1)!="@"&&targetVal.indexOf("@")==-1)) {
					$(item).attr({'data-html':''});
					// 给隐藏域赋值
		...
		if(curIndex > -1){
			...
			if(_cache.onlyFlag) {
				...
				for(var i = 0; i < _config.mailArr.length; i++) {
					...
					wrap += '<li class="p-index'+i+'" style="color:#000000;padding-top:3px;padding-bottom:3px;padding-left:5px;">'+'<font class="output-num"></font><em class="em" data-html="'+_config.mailArr[i]+'" style="font-style:normal;color:#000000;">'+_config.mailArr[i]+'</em></li>';
				}
				wrap += '</ul>';
				_cache.onlyFlag = false;
				$(cfg.parentNode).append(wrap);
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.height,'width':cfg.width - 43 + 'px','left':18 + 'px',
