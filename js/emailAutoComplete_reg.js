 ...
 */
 ...
 function EmailAutoComplete(options) {
	...
	this.config = {
		targetCls      :  '.email_txt',       // 目标input元素
		parentCls      :  '.email_p',       // 当前input元素的父级类
		hiddenCls      :  '.email_p',       // 当前input隐藏域 
		//searchForm     :  '.jqtransformdone', //form表单
		hoverBg        :  'email_selected',          // 鼠标移上去的背景
		//inputValColor  :  'dark',              // 输入框输入提示颜色
		mailArr        : ["@163.com","@gmail.com","@qq.com","@126.com","@hotmail.com","@yahoo.com","@yahoo.com.cn","@live.com","@sohu.com","@sina.com"], //邮箱数组
	...
	init: function(options){
		...
		$(_config.targetCls).each(function(index,item){
			...
			$(item).keyup(function(e){	
				...
				// 如果输入框值为空的话 那么下拉框隐藏
				//||(targetVal.substring(targetVal.length-1)!=""&&targetVal.indexOf("")==-1)
				//不需要输入@即可出现有限提示故删除即可 ul li中需要添加list-style-type:none
				if(targetVal == '') {
					$(item).attr({'data-html':''});
					// 给隐藏域赋值
		...
		if(curIndex > -1){
			...
			if(_cache.onlyFlag) {
				...
				for(var i = 0; i < _config.mailArr.length; i++) {
					...
					wrap += '<li class="p-index'+i+'" style="list-style-type:none;color:#000000;padding-top:3px;padding-bottom:3px;padding-left:5px;">'+'<font class="output-num"></font><em class="em" data-html="'+_config.mailArr[i]+'" style="font-style:normal;color:#000000;">'+_config.mailArr[i]+'</em></li>';
				}
				wrap += '</ul>';
				_cache.onlyFlag = false;
				$(cfg.parentNode).append(wrap);
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.height,'width':cfg.width - 43 + 'px','left':133 + 'px',
					'border':'1px solid #ccc','z-index':10000,'background':'white'});
			}
		...
		if(/@/.test(curVal)) {
			...
			if(newArrs.length > 0) {
				...
				for(var j = 0, jlen = newArrs.length; j < jlen; j++) {
					html += '<li class="p-index'+j+'" style="list-style-type:none;color:#000000;padding-top:3px;padding-bottom:3px;padding-left:5px;">'+'<font class="output-num"></font><em class="em" data-html="'+newArrs[j]+'" style="font-style:normal;color:#000000;">'+newArrs[j]+'</em></li>';
				}
